...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editTodo(_, data){
        this.todos.forEach(e => {
          if(e.id === data.id) e.title = data.value;
        });
      }
    },
    mounted(){
      ...
      this.pubEditTodoId = pubsub.subscribe('editTodo', this.editTodo);
    },
    beforeDestroy(){
      ...
      pubsub.unsubscribe(this.pubEditTodoId);
    },
    watch:{
      todos:{
        deep: true,
...
<style>
  ...
  .btn-edit{
    ...
    border: 1px solid rgb(67, 168, 208);
    margin-right: 5px;
  }
  .btn-edit:hover{
    color: #fff;
